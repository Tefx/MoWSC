...
splitMutate p o h = splitHost o h <$> (randPos 2 . (+1) . IntSet.size $ _tasks h)

splitMutateR::(RandomGen g)=>Problem->InsHost->Rand g [InsHost]
splitMutateR p (IH t ts) = do pb <- getRandomR (0.3, 0.7)
                              let tsl = IntSet.toAscList ts
                              (tsl0, tsl1) <- partition snd <$>
                                              mapM (doWithProb pb (,True) (,False)) tsl
                              return [ IH t . IntSet.fromAscList $ map fst tsl0
                                     , IH t . IntSet.fromAscList $ map fst tsl1]
...
instance Chromosome C2v3 where
  ...
  mutate p cur (C2v3 o is) = do
    ...
    is' <- (join $ doWithProb 1 f return is) >>=
           --join . doWithProb 0.95 (mutateByDriftting p) return >>=
           join . doWithProb 1 (mutateInTypes p) return
    o' <- mutateOrder p o
...
mutateBySplitting p o is = do
  ph <- getRandomR (0, Vec.length is-1)
  [h0, h1] <- splitMutateR p $ is ! ph
  return $ if (IntSet.null $ _tasks h0) || (IntSet.null $ _tasks h1)
           then is
